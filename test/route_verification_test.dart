...
void main() {
  ...
  test(
    'Verify Route Calculation and Walking Instructions between Siam and MBK',
    () async {
      ...
      for (int i = 0; i < result.segments.length; i++) {
        ...
        print(
          '  Segment $i: ${s.lineId} (${s.lineName}) from ${s.fromStation.id} to ${s.toStation.id}',
        );
        print(
          '    Exit: ${s.exit?.exitCode}, Path points: ${s.walkingPath?.length}',
        );
      }
      ...
      for (int i = 0; i < segment.walkingPath!.length; i++) {
        print(
          '  Point $i: ${segment.walkingPath![i].latitude}, ${segment.walkingPath![i].longitude}',
        );
      }
  ...
  test(
    'Verify Route Calculation and Walking Instructions between National Stadium and MBK',
    () async {
      TestWidgetsFlutterBinding.ensureInitialized();
  ...
  test(
    'Verify Route Calculation and Walking Instructions between Charoen Nakhon and ICONSIAM',
    () async {
      TestWidgetsFlutterBinding.ensureInitialized();
      ...
      final iconsiamLandmark = repo.landmarks.firstWhere(
        (l) => l.id == 'LM_ICONSIAM',
      );
  ...
  test(
    'Verify Route Calculation and Walking Instructions between Chit Lom and CentralWorld',
    () async {
      TestWidgetsFlutterBinding.ensureInitialized();
      final container = ProviderContainer();
      final repo = container.read(transitRepositoryProvider);
      ...
      final centralWorldLandmark = repo.landmarks.firstWhere(
        (l) => l.id == 'LM_CENTRALWORLD',
      );
  ...
  test(
    'Verify Route Calculation and Walking Instructions between Phrom Phong and EmSphere',
    () async {
      TestWidgetsFlutterBinding.ensureInitialized();
      final container = ProviderContainer();
      final repo = container.read(transitRepositoryProvider);
      ...
      final emSphereLandmark = repo.landmarks.firstWhere(
        (l) => l.id == 'LM_EMSPHERE',
      );
      ...
      expect(
        segment.instructionsEn,
        contains('EmSphere via the elevated skywalk'),
      );
    },
  );

  test(
    'Verify Route Calculation and Walking Instructions to Siam Paragon',
    () async {
      TestWidgetsFlutterBinding.ensureInitialized();
      final container = ProviderContainer();
      final repo = container.read(transitRepositoryProvider);
      ...
      final w1Station = repo.getStation('BTS_W1');
      final paragonLandmark = repo.landmarks.firstWhere(
        (l) => l.id == 'LM_PARAGON',
      );
      ...
      expect(
        state.routeResult!.segments.first.instructionsTh,
        contains('สยามพารากอน (ทางออก 3, 5 หรือ 6)'),
      );
      ...
      expect(
        state.saverRoute!.segments.first.instructionsTh,
        contains('Siam Discovery และ Siam Center'),
      );
    },
  );
}
