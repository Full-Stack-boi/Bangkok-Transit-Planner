...
void main() {
  ...
  group('Transit Routing (Dijkstra Algorithm) Tests', () {
    ...
    setUp(() {
      ...
      graph.addStation(const Station(
        id: 'BTS_A',
        code: 'N1',
        nameTh: 'สถานีเอ',
        nameEn: 'Station A',
    ...
    test('Should find direct route on the same line without transfers', () {
      ...
      expect(result.transferCount, equals(0)); // ต้องไม่มีการเปลี่ยนสาย (0 transfer)
      expect(result.totalWeight, equals(2.0)); // เวลาเดินทางรวมต้องเป็น 2.0 นาที
    });

    test('Should calculate correct route and transfer details between different lines', () {
      ...
      expect(result!.stationCount, equals(4)); // เส้นทางที่สั้นที่สุดต้องผ่าน A -> B -> C -> D (4 ขั้นตอน)
      expect(result.transferCount, equals(1)); // ต้องมีการเปลี่ยนสาย 1 ครั้ง (จาก BTS ไป MRT)
    ...
    test('Should return weight of 0 and same station when origin and destination are identical', () {
    ...
    test('Should return null if there is no possible path between stations', () {
