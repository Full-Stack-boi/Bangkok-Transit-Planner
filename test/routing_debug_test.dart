...
void debugRoute(
  ...
  String fromId,
  String toId, {
  List<String>? expectedManualPath,
}) {
  ...
  for (final step in result!.path) {
    ...
    print(
      '  ${s?.nameEn ?? step.stationId} (${step.stationId}) [${step.lineId}]${isTransfer ? " *** TRANSFER ***" : ""}',
    );
  ...
  if (expectedManualPath != null && expectedManualPath.isNotEmpty) {
    ...
    for (int i = 0; i < expectedManualPath.length - 1; i++) {
      ...
      if (e != null) {
        ...
        print(
          '  -> ${s?.nameEn ?? expectedManualPath[i + 1]}: +${weight.toStringAsFixed(2)} min [${e.lineId}]',
        );
      } else {
        print(
          '  -> ${expectedManualPath[i + 1]}: NO EDGE FOUND from ${expectedManualPath[i]} !!!',
        );
      }
    }
    print('Expected path total: ${manual.toStringAsFixed(2)} min');
    print(
      'Dijkstra savings vs expected: ${(manual - result.totalWeight).toStringAsFixed(2)} min',
    );
  }
  print('\n');
}
